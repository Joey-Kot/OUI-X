...
export const verifyUserMCPToolServerConnection = async (token: string, connection: object) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/users/user/mcp/tool_servers/verify`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const registerUserMCPOAuthClient = async (
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/users/user/mcp/oauth/clients/register`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
