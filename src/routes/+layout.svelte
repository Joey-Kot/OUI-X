<script>
	...
	const chatEventHandler = async (event, cb) => {
		...
		if ((event.chat_id !== $chatId && !$temporaryChatEnabled) || isFocused) {
			...
			if (type === 'execute:python') {
			...
			} else if (type === 'request:chat:completion' || type === 'request:responses:completion') {
				console.log(data, $socket.id);
				const { session_id, channel, form_data, model } = data;
				const expectsResponses = type === 'request:responses:completion';
				...
				try {
					...
					if (directConnections) {
						...
						try {
							if (expectsResponses && endpointType !== 'responses') {
								throw new Error('Provider does not support /responses endpoint');
							}
							if (!expectsResponses && endpointType === 'responses') {
								throw new Error('openai_responses provider must use /responses endpoint');
							}
							...
							const [res, controller] =
								expectsResponses
									? await responsesCompletion(OPENAI_API_KEY, form_data, OPENAI_API_URL)
									...
									const processStream = async () => {
										while (true) {
											...
											while (streamBuffer.includes('\n')) {
												...
												const line = streamBuffer.slice(0, newlineIndex).trimEnd();
